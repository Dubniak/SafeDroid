...
def generate_Pkg_List(x):
	'''
		@param x 		: 	a VMAnalysis instance

		@rtype			:	A set of package names

	'''
	pkg_list = [k[1] for k in x.get_tainted_packages().get_packages()]
	return pkg_list

def generate_API_List(x,pkg):
	'''
		@param x 		: a VMAnalysis instance
		@param pkg_set 	: a package for which APIs need to be extracted

		@rtype 			: A set of (pkg,api)

	'''
	API_methods = x.get_methods_for_package(pkg,"",".")
	return API_methods
